/*
 * @Author: Ceoifung
 * @Date: 2023-08-04 14:53:33
 * @LastEditors: Ceoifung
 * @LastEditTime: 2023-08-04 14:54:00
 * @Description: XiaoRGEEK All Rights Reserved. Copyright © 2023
 */
#include "pxt.h"

namespace XiaoRGEEKIR { 
int ir_code = 0x00;
int ir_addr = 0x00;
int data;
...
int logic_value(){//Determine logical value "0" and "1" subfunctions
    ...
    uint32_t nowtime;
    while(!uBit.io.P8.getDigitalValue());//Low wait
    nowtime = system_timer_current_time_us();
    if((nowtime - lasttime) > 400 && (nowtime - lasttime) < 700){//Low 560us
        while(uBit.io.P8.getDigitalValue());//Is high wait
...
void remote_decode(void){
    ...
    uint32_t lasttime = system_timer_current_time_us();
    uint32_t nowtime;
    while(uBit.io.P8.getDigitalValue()){//High level wait
        nowtime = system_timer_current_time_us();
    ...
    if((nowtime - lasttime) < 10000 && (nowtime - lasttime) > 8000){//9ms
        while(uBit.io.P8.getDigitalValue());//High wait
        lasttime = system_timer_current_time_us();
